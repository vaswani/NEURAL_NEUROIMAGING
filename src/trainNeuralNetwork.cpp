...
{ 
    ...
    if (use_mmap_file == false) {
      ...
      if (randomize == true) {
        ...
        while (counter < training_data_size) {
          ...
          vector<int> temp(0,vector_size*myParam.ngram_size);
    ...
    if (use_mmap_file == false) {
      ...
      training_data = Map< Matrix<int,Dynamic,Dynamic> >(training_data_flat_mmap->data(), myParam.ngram_size, training_data_size);
    }
    ...
    for (data_size_t train_id=0; train_id < training_data_size; train_id++)
    {
        ...
        if (use_mmap_file == false) {
          ...
          int output_word = training_data_flat_mmap->at((train_id+1)*myParam.ngram_size - 1);
        }
	    ...
	    Matrix<int,Dynamic,Dynamic> minibatch;// = training_data.middleCols(minibatch_start_index, current_minibatch_size);
            ...
            //now reading the ngrams from the mmaped file
              for (int k=0; k<ngram_size; k++){
                for (data_size_t index = minibatch_start_index; index<minibatch_start_index+current_minibatch_size; index++) {
                  minibatch(k,index) = training_data_flat_mmap->at(index*ngram_size+k);
                }
              }
            } else {
